...
config/authorized_keys.example # 复制成 config/authorized_keys（gitignored）
```
...
2. **SSH 端口会从 22 改到 60022。** 在动手前确认 60022 可达：
   ...
   - 当前用的是哪个端口、从哪登录，先记下来。
3. 准备 SSH 公钥列表（否则 ssh-hardening 在实跑时会交互索要）：
   ```bash
   cp config/authorized_keys.example config/authorized_keys
   # 编辑：每行一个公钥，确保包含你当前登录用的 key
   ```
   注意：本 kit 已改为 **authorized_keys 已存在则不覆盖**，但 beelink 若还没有该文件，
   实跑会写入；务必确认包含你正在用的 key。
